...
async function saveData(data) {
  // We REMOVED the line: const { customer_name, customer_email } = data;
  ...
  try {
    ...
    // NEW: Convert the entire data object to a single string
    const allDataString = JSON.stringify(data);
    
    // NEW: The new row is just ONE cell
    const newRow = [
      allDataString 
    ];
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: 'Sheet1!A:A', // NEW: Only append to Column A
      valueInputOption: 'USER_ENTERED',
      resource: {
        values: [newRow],
      },
    });
    console.log('Data saved to Google Sheet successfully (as one cell)!');
  } catch (error) {
